...
C        Centre of pixel square is (XMID, YMID), circle of radius RODRAD
C        totally contains pixel square. RODRAD is thus radius current rod.
         ...
         YMID=FLOAT(PY(TOP)-384)/XYSCAL+EBY2
...
C
      ...
      ELSE
         ANGLE=ATAN(AY/AX)
         IF(AX.LT.0.0) ANGLE=ATAN(AY/AX)+PI
      ENDIF
